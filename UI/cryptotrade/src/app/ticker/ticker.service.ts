...
export class TickerService {
  ...
  getQuoteStream(fromSym: string): Observable<Ticker[]> {
    ...
    return Observable.create((observer) => {
      ...
      const url = AppSetting.DATA_RETRIEVER + fromSym;
      ...
      eventSource.onmessage = (event) => {
        ...
        json.forEach(element => {
          ...
          this.ticker.push(new Ticker(element['currency'],
            element['rate'],
            element['change'], element['openDay'], element['highDay'], element['lowDay'] ));
        });
  ...
  // }
  getHistStream(fromSym: string, toSym: string): Observable<any> {
    ...
    return Observable.create((observer) => {
      ...
      const url = 'http://localhost:8305/crypto/histdata/' + fromSym  + '/' + toSym;
      ...
      const json = JSON.parse(event.data);
        ...
        // this.ticker.push(new Ticker(json['currency'], json['rate']));
      this.ngZone.run(() => observer.next(json));
    };
      eventSource.onerror = (error) => {
        // readyState === 0 (closed) means the remote source closed the connection,
        // so we can safely treat it as a normal situation. Another way of detecting the end of the stream
